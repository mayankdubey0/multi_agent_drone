...
with mujoco.viewer.launch_passive(model, data) as viewer:
    while viewer.is_running():
        ...
        # All motors nominally equal thrust
        base_thrust = 4

        # Add yaw control (positive = spin right)
        yaw_torque = 2

        # Adjust rotor thrusts to induce yaw
        data.ctrl[:4] = np.array([
            base_thrust + yaw_torque,   # rotor 0 (CW)
            base_thrust - yaw_torque,   # rotor 1 (CCW)
            base_thrust + yaw_torque,   # rotor 2 (CW)
            base_thrust - yaw_torque    # rotor 3 (CCW)
        ])
